...
from odoo.addons.services.models.service_order import SERVICE_ORDER_STATE


class ServiceReport(models.Model):
    _name = "service.report"
    _description = "Service Analysis Report"
    _auto = False
    _rec_name = 'date'
    _order = 'date desc'
    ...
    team_id = fields.Many2one(comodel_name='crm.team', string="Service Team", readonly=True)
    user_id = fields.Many2one(comodel_name='res.users', string="Serviceperson", readonly=True)
    state = fields.Selection(selection=SERVICE_ORDER_STATE, string="Status", readonly=True)
    ...
    order_reference = fields.Reference(string='Related Order', selection=[('service.order', 'Services Order')], group_operator="count_distinct")
    ...
    def _query(self):
        with_ = self._with_service()
        return f"""
            {"WITH" + with_ + "(" if with_ else ""}
            SELECT {self._select_service()}
            FROM {self._from_service()}
            WHERE {self._where_service()}
            GROUP BY {self._group_by_service()}
            {")" if with_ else ""}
        """
