...
class ServiceAdvancePaymentInv(models.TransientModel):
    _name = 'service.advance.payment.inv'
    ...
    service_order_ids = fields.Many2many(
        'service.order', default=lambda self: self.env.context.get('active_ids'))
    ...
    amount_to_invoice = fields.Monetary(
        ...
        compute="_compute_invoice_amounts",
        help="The amount to invoice = service Order Total - Confirmed Down Payments.")
    ...
    def _compute_count(self):
        for wizard in self:
            wizard.count = len(wizard.service_order_ids)

    @api.depends('service_order_ids')
    def _compute_has_down_payments(self):
        for wizard in self:
            wizard.has_down_payments = bool(
                wizard.service_order_ids.order_line.filtered('is_downpayment')
            )
    ...
    def _compute_currency_id(self):
        ...
        for wizard in self:
            if wizard.count == 1:
                wizard.currency_id = wizard.service_order_ids.currency_id
    ...
    def _compute_company_id(self):
        ...
        for wizard in self:
            if wizard.count == 1:
                wizard.company_id = wizard.service_order_ids.company_id
    ...
    def _compute_product_id(self):
        ...
        for wizard in self:
            if wizard.count == 1:
                wizard.product_id = wizard.company_id.service_down_payment_product_id
    ...
    def _compute_display_invoice_amount_warning(self):
        for wizard in self:
            ...
            if wizard.advance_payment_method == 'percentage':
                invoice_amount = wizard.amount / 100 * sum(wizard.service_order_ids.mapped('amount_total'))
    ...
    def _compute_display_draft_invoice_warning(self):
        for wizard in self:
            wizard.display_draft_invoice_warning = wizard.service_order_ids.invoice_ids.filtered(lambda invoice: invoice.state == 'draft')

    @api.depends('service_order_ids')
    def _compute_invoice_amounts(self):
        for wizard in self:
            wizard.amount_invoiced = sum(wizard.service_order_ids._origin.mapped('amount_invoiced'))
            wizard.amount_to_invoice = sum(wizard.service_order_ids._origin.mapped('amount_to_invoice'))
    ...
    def _check_down_payment_product_is_valid(self):
        for wizard in self:
            if wizard.count > 1 or not wizard.product_id:
                continue
            if wizard.product_id.service_invoice_policy != 'order':
                raise UserError(_(
    ...
    def create_invoices(self):
        ...
        invoices = self._create_invoices(self.service_order_ids)
        return self.service_order_ids.action_view_invoice(invoices=invoices)
    ...
    def view_draft_invoices(self):
        return {
            ...
            'domain': [('line_ids.service_line_ids.order_id', 'in', self.service_order_ids.ids), ('state', '=', 'draft')],
        }

    #=== BUSINESS METHODS ===#

    def _create_invoices(self, service_orders):
        self.ensure_one()
        if self.advance_payment_method == 'delivered':
            return service_orders._create_invoices(final=self.deduct_down_payments, grouped=not self.consolidated_billing)
        else:
            self.service_order_ids.ensure_one()
            self = self.with_company(self.company_id)
            order = self.service_order_ids

            # Create deposit product if necessary
            if not self.product_id:
                self.company_id.service_down_payment_product_id = self.env['product.product'].create(
                    self._prepare_down_payment_product_values()
                )
                self._compute_product_id()

            # Create down payment section if necessary
            ServiceOrderline = self.env['service.order.line'].with_context(service_no_log_for_new_lines=True)
            if not any(line.display_type and line.is_downpayment for line in order.order_line):
                ServiceOrderline.create(
                    self._prepare_down_payment_section_values(order)
                )

            down_payment_lines = ServiceOrderline.create(
