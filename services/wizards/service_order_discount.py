...
class ServiceOrderDiscount(models.TransientModel):
    _name = 'service.order.discount'
    _description = "Discount Wizard"

    service_order_id = fields.Many2one(
        'service.order', default=lambda self: self.env.context.get('active_id'), required=True)
    company_id = fields.Many2one(related='service_order_id.company_id')
    currency_id = fields.Many2one(related='service_order_id.currency_id')
    ...
    def _get_discount_product(self):
        ...
        self.ensure_one()
        discount_product = self.company_id.service_discount_product_id
        if not discount_product:
            self.company_id.service_discount_product_id = self.env['product.product'].create(
    ...
    def _create_discount_lines(self):
        ...
        if self.discount_type == 'amount':
            ...
            for line in self.service_order_id.order_line:
        ...
        return self.env['service.order.line'].create(vals_list)

    def action_apply_discount(self):
        self.ensure_one()
        self = self.with_company(self.company_id)
        if self.discount_type == 'sol_discount':
            self.service_order_id.order_line.write({'discount': self.discount_percentage*100})
        else:
